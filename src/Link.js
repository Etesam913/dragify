...
function Link(props) {
   ...
   if (deleted) {
      ...
      return (
         <Component fontColor={props.darkMode ? "rgb(232, 230, 227)" : "black"} ref={component} initial={{opacity: 0}} animate={controls} style={{ scale }} drag={props.canEdit ? true : false} onDragEnd={()=>{storeTranslations()}} whileHover={{zIndex: 2}} dragMomentum={false} dragConstraints={props.canvas} onHoverStart={() => { setHover(true) }} onHoverEnd={() => {setHover(false)}} onDragEnd={()=>{storeTranslations()}}>
