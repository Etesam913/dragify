    ...
    if (stage === 0) { // Border radius, 0, 20, 100
      return (
        <Box className={drag ? "box-shadow" : ""} backgroundColor={props.darkMode ? "rgb(39, 39, 39)" : "rgb(90%, 90%, 90%);"}
          animate={{ borderRadius: "0%" }} transition={{ duration: 1 }} >
          <Header> Paste Link Below</Header>
          <Subtitle> ex: www.reddit.com </Subtitle>
          <LinkInput className="cursor-text" ref={linkInput} fontColor={props.darkMode ? "rgb(232, 230, 227)" : "black"}
            buttonBackgroundColor={props.darkMode ? "rgb(32, 34, 35)" : "rgb(80%, 80%, 80%)"}>
          </LinkInput>
          <Button className="cursor-auto" buttonBackgroundColor={props.darkMode ? "rgb(32, 34, 35)" : "rgb(80%, 80%, 80%)"}
            fontColor={props.darkMode ? "rgb(232, 230, 227)" : "black"}
            onClick={() => { storeLink() }} whileHover={{ scale: 1.1 }} whileTap={{ scale: 0.9 }}>Next</Button>
        </Box>
      );
    }
    else if (stage === 1) {
      ...
      return (
        <Box className={drag ? "box-shadow" : ""} backgroundColor={props.darkMode ? "rgb(39, 39, 39)" : "rgb(90%, 90%, 90%);"}
          animate={{ borderRadius: "100%" }} transition={{ duration: 3 }}>
          {loading ? <h3> loading </h3> : <a style={{ borderRadius: "100%" }} href={linkAddress}>
            <Logo src={image} whileHover={{ scale: 1.1 }} whileTap={{ scale: 0.9 }}></Logo></a>}
        </Box>
      );
    }
    else {
  ...
  if (deleted) {
    ...
    return (
      <Component className={drag ? "cursor-dragging" : "cursor-drag"} fontColor={props.darkMode ? "rgb(232, 230, 227)" : "black"} ref={component}
        ...
        drag={props.canEdit ? true : false} onDragEnd={() => { storeTranslations() }} whileHover={{ zIndex: 2 }}
        dragConstraints={props.canvas} onHoverStart={() => { setHover(true) }} onHoverEnd={() => { setHover(false) }} dragMomentum={false}
